...
for i in range(preprocessed_image.shape[0]):
    if(i == 136):
        print(preprocessed_image[i].reshape(3, 3))
        print(kernel)
        print(preprocessed_image[i].reshape(3, 3) * kernel.reshape(3, 3))
    conv_result = np.sum(preprocessed_image[i].reshape(3, 3) * kernel.reshape(3, 3)) + bias
    if(i == 136):
        print(conv_result)
...
fc_weights = loaded_weights[0][0].reshape(676, 10)  
